...
def parse_args():
    ...
    p.add_argument("--w_acc", type=float, default=0.60)
    p.add_argument("--w_vec_g", type=float, default=0.16)
    p.add_argument("--w_vec_l", type=float, default=0.16)
    p.add_argument("--w_dep", type=float, default=0.08)
    p.add_argument("--dep_blur_sigma", type=float, default=3.0)
    p.add_argument("--dep_shuffle_grid", type=int, default=4)
    p.add_argument("--n_rois", type=int, default=9)
...
def main():
    ...
    gamma = 2.0
    weights = (0.5 + 0.5 * scores).pow(gamma)
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(tqdm(train_loader, initial=step_global, desc=f"Epoch {epoch}")):
            ...
            if (images_raw is not None) and (prompt_tti is None):
                raise RuntimeError("Batch is missing token_type_ids; Please provide in collate_fn (consistent with SFT).")
            ...
            if images_raw is None:
                images_for_gen = None
                tti_for_gen = torch.zeros_like(prompts, device=device)
            else:
            ...
            with torch.no_grad():
                for i in range(args.k):
                    set_seed_time()
                    outputs = accelerator.unwrap_model(model).generate(**gen_inputs, **gen_kwargs)
                    comp_i = outputs[:, P:]
            ...
            comp_mask = first_eos_mask(comp_cat, eos_id=eos_id).to(prompts.dtype)

            comp_texts: List[str] = [
                tokenizer.decode(row, skip_special_tokens=True).strip()
                for row in comp_cat
            ]
            assert len(comp_texts) == B * args.k, f"Comp_texts length exception：{len(comp_texts)} vs {B*args.k}"

            prompts_k   = prompts.repeat(args.k, 1)
            prompt_attn_k = prompt_attn.repeat(args.k, 1)
            if images_raw is None:
                prompt_tti_k = torch.zeros_like(prompts_k, device=device)
            else:
                prompt_tti_k = prompt_tti.repeat(args.k, 1).to(device)

            comp_tti = torch.zeros_like(comp_cat, device=device)

            pc_ids  = torch.cat([prompts_k, comp_cat.to(device)], dim=1)
            pc_attn = torch.cat([prompt_attn_k.to(device), comp_mask.to(device)], dim=1)
            pc_tti  = torch.cat([prompt_tti_k, comp_tti], dim=1)
            ...
            if images_for_gen is None:
                ...
                assert len(images_k) == B * args.k, f"Images_k length exception：{len(images_k)} vs {B*args.k}"
            else:
            ...
            with torch.no_grad():
                ...
                with accelerator.unwrap_model(model).disable_adapter():
                    ...
                    logp_ref   = logp_ref[:, start:]

            if kl_coef_this_step > 0.0:
                delta = (logp_ref - logp_actor)
            ...
            assert logp_actor.size() == per_tok_kl.size() == (B * args.k, Lmax), \
                f"logp/kl abnormal shape：{logp_actor.size()} vs {per_tok_kl.size()}, expectation {(B*args.k, Lmax)}"
            ...
            advantages = group_norm_and_clip(rewards_mix, clip=args.adv_clip)
            adv_flat = advantages.reshape(B * args.k, 1)

            imp_weight = torch.exp(logp_actor - logp_actor.detach())
            per_token_loss = imp_weight * adv_flat
            per_token_loss = -(per_token_loss - kl_coef_this_step * per_tok_kl)

            comp_mask_f = comp_mask.to(per_token_loss.dtype)
            loss_per_seq = (per_token_loss * comp_mask_f).sum(dim=1) / (comp_mask_f.sum(dim=1) + 1e-8)
            ...
            if (step_global + 1) % args.save_step == 0:
                ...
                if args.log_path and accelerator.is_main_process:
                    try:
                        ...
                        logger.warning(f"Writing log failed: {e}")
